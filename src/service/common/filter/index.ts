...
const _apply = (i: 鉢, filter: FilterState): boolean => {
  ...
  if (isDefined(keyword)) {
    ...
    const words = k
      ...
      .flatMap(i => i.split('　'))
      .filter(Boolean);
    const is = words.every(w => [i.詳細.科, i.詳細.属, i.詳細.種名, i.name].filter(Boolean).join('').includes(w));
    if (!is) return false;
  }
  if (isDefined(日光の強度)) {
